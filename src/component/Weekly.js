...
const Weekly = ({ cityInfo }) => {
	...
	useEffect(() => {
		const getWeeklyData = async () => {
			const url = `/.netlify/functions/getWeekly?lat=${lat}&lon=${lon}`;
			...
			{weekly.map((day, index1) => {
				return (
					<React.Fragment>
						<div
							key={index1}
							className="w-4/5 mx-auto md:w-48 mt-5  text-center shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px] backdrop-blur-3xl"
						>
